...
export function TripDetail() {
  ...
  const handleCancelRide = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Раньше здесь не было ничего: ни обновления, ни сброса флага —
      // расчёт был на то, что придёт realtime-событие об изменении поездки.
      // Но постоянное соединение на бесплатном тарифе Supabase не гарантия
      // (в ленте и уведомлениях от него как раз отказались в пользу опроса),
      // и если канал не поднялся, кнопка оставалась в состоянии загрузки до
      // перезагрузки страницы, хотя поездка уже была отменена. Забираем
      // свежее состояние сами — ровно как в соседних обработчиках.
      await fetchRide();
    } catch (err: any) {
